...
//| label: chart-vote-shares
{
  ...
  const formatDiff = d =>
    d.diff == null
      ...
      ? "±0"
      : `${d.diff >= 0 ? "+" : ""}${d.diff.toLocaleString("de-DE", {
...
viewof hoveredRow = {
  ...
  const plot = Plot.plot({
    ...
    marks: [
      ...
      // dots are the whole series and have to read as such: solid fill, larger
      // radius, and a white stroke to keep polls that land close together apart.
      Plot.dot(raw, {
        x: "date",
        y: "percent",
        fill: "label",
        ...(showLine
          ...
          : { fillOpacity: 0.9, r: 4, stroke: "white", strokeWidth: 0.75 })
      }),
      ...(showLine ? [
...
//| label: chart-poll-trend-shares
{
  ...
  const maxY = Math.max(35, Math.ceil((d3.max(cur_polls.history, d => d.percent) + 3) / 5) * 5)
