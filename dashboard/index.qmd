...
cleanCoalitionHistory = rows => {
  ...
  // first collapse exact (date, label) duplicates by averaging — a pooled
  // result can occasionally get recomputed more than once for the same poll
  // date — mirroring the dedupe step in cleanCoalitions()
  ...
  // then sum across the different party orderings of the same coalition
  // (mutually exclusive "who leads" outcomes) into one row per coalition_key/date
  return Array.from(byDateLabel, ([date, byLabel]) => {
    const byKey = d3.rollup(
      Array.from(byLabel, ([label, probability]) => ({ coalition_key: normCoalition(label), probability })),
      v => d3.sum(v, d => d.probability),
      d => d.coalition_key
    )
    return Array.from(byKey, ([coalition_key, probability]) => ({
      coalition_key,
      date: new Date(date),
      probability
    }))
...
viewof selectedCoalitions = {
  const options = overview_coalitions.slice().sort((a, b) =>
    d3.descending(a.probability, b.probability) || d3.descending(a.vote_share, b.vote_share)
  )
  const active  = new Set(options.length ? [options[0].coalition_key] : [])
  ...
  for (const d of options) {
    ...
    style(btn, color, active.has(d.coalition_key))

    btn.onclick = () => {
      if (active.has(d.coalition_key)) {
        active.delete(d.coalition_key)
      } else {
        active.add(d.coalition_key)
      }
      style(btn, color, active.has(d.coalition_key))
...
viewof hoveredCoalitionRow = {
  ...
  const active = rows.filter(d => selectedCoalitions.includes(d.coalition_key))

  const labelByKey = Object.fromEntries(overview_coalitions.map(d => [d.coalition_key, d.label]))
  const colorByKey = Object.fromEntries(overview_coalitions.map(d => [d.coalition_key, party_colors[d.first_party] || "#aaa"]))

  const selectedLabels = selectedCoalitions.map(k => labelByKey[k]).filter(Boolean)
  ...
  const byDate = d3.rollup(
    active,
    v => Object.fromEntries(v.map(d => [d.coalition_key, d.probability])),
    d => +d.date
  )
    ...
    marks: [
      ...
      ...(showLine ? [
        Plot.line(active, {
          x: "date",
          y: "probability",
          stroke: d => labelByKey[d.coalition_key],
          z: "coalition_key",
          strokeWidth: 2
        })
      ] : []),
      Plot.dot(active, {
        x: "date",
        y: "probability",
        fill: d => labelByKey[d.coalition_key],
  ...
  const items = selectedCoalitions.map(key => {
    const g    = overlay.append("g")
    const dot  = g.append("circle").attr("r", 4).attr("fill", colorByKey[key] || "#aaa")
    const text = g.append("text").attr("dx", 8).attr("dy", "0.32em")
      .attr("fill", colorByKey[key] || "#aaa").attr("font-size", 13).attr("font-weight", "bold")
      .attr("paint-order", "stroke")
      .attr("stroke", "white").attr("stroke-width", 3).attr("stroke-linejoin", "round")
    return { key, label: labelByKey[key], dot, text }
  })
