...
normalizeDensityCurve = group => {
  const xs = group?.seat_share ?? []
  const ys = group?.density ?? []
...
plotCoalitionDensity = (group, height = 260) => {
  ...
  const probMajority = densityValue(group?.prob_majority)
...
cur = Object.fromEntries(          // Überblick + sidebar, ~77 KB
  await Promise.all(
    ["coalitions", "party_data", "hurdle_data", "pollster_data"]
      .map(async key => [key, await election_files[election][key].json()])
  )
)

cur_polls        = election_files[election].history_data.json()       // Deskriptive Analyse
...
//| label: chart-coalition-densities
{
  ...
  const densityByLabel = new Map(source.map(d => [d.label, d.coalition]))
