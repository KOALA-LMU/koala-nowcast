...
<!-- tall enough for the selector with a second party combination opened -->
## Row {height=1010px}
...
viewof hoveredCoalitionRow = {
  ...
  // both selections can be led by the same party, and would then be two lines in
  // the same colour; the second one is dashed so they stay tellable apart
  series.forEach((d, i) => {
    d.dash = series.slice(0, i).some(other => other.color === d.color) ? "6,4" : undefined
  })
...
viewof selectedCoalitions = {
  // Up to two hand-built party combinations, each one a set of parties plus the
  ...
  const render = () => {
    ...
    // mirrors the chart: two combinations in the same colour are told apart by
    // dashing the second one, and the swatch shows which line is which
    const drawn = selections.filter(selection => selection.parties.size)
    const dashOf = selection => {
      const i = drawn.indexOf(selection)
      return i > 0 && drawn.slice(0, i).some(other => colorOf(other) === colorOf(selection)) ? "6,4" : ""
    }
