...
viewof hoveredRow = {
  ...
  const byDateRaw = d3.rollup(
    history_raw.filter(d => new Date(d.date) >= cutoff),
    v => Object.fromEntries(v.map(d => [d.party, d.percent])),
    d => +new Date(d.date)
  )
