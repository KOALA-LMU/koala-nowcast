...
resources:
  - berd-nfdi-logo.svg
  # Blog posts are rendered before the dashboard; copy their self-contained
  # HTML and the preview images used by the static post list.
  - blog/**/index.html
  - blog/**/*.png
---
...
/* Plot renders a figure's title and subtitle as sibling headings, so the space
   between them is h2's padding plus the larger of the two margins — some 26px,
   which read as an empty band under the rule. Tighten it so the subtitle sits
...
/* The Stimmenanteile card sets the election name above Plot's own title, and
   the two read as one block: drop the space a heading normally opens with. */
.shares-header figure > h2 {
  margin-top: 0 !important;
}
...
/* Zeitverlauf party-combination picker. The colours of the pills are per party
   and stay inline; everything structural lives here, so the states a style
   attribute cannot express (:hover, :focus) are available. */
.kombi-picker {
  display: flex;
...
.kombi-card__name--empty {
  font-weight: 400;
  font-style: italic;
  color: #8a9099;
}

/* pushes the leading-party box to the right-hand end of the header row */
.kombi-card__spacer {
  flex: 1 1 auto;
}
...
/* the native arrow sits too far from the text inside a pill-shaped box, so the
   control draws its own (encoded as a data URI, hence the %23 for #) */
.kombi-leader select {
  appearance: none;
  ...
  border-radius: 999px;
  background: white url("data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 10 6'%3E%3Cpath d='M1 1l4 4 4-4' fill='none' stroke='%236c7278' stroke-width='1.5' stroke-linecap='round'/%3E%3C/svg%3E") no-repeat right 10px center;
  background-size: 9px;
  cursor: pointer;
}

.kombi-leader select:hover {
  border-color: #b9bfc5;
}
...
/* removes one combination card; a bare glyph rather than a labelled button, so
...
/* The blog is global dashboard content. It is plain HTML rather than an OJS
   cell, so changing the selected election never rebuilds or replaces it. */
.blog-index {
  --blog-ink: #1f2933;
  --blog-muted: #6b7280;
  --blog-accent: #2b6cb0;
...
/* The Überblick charts have fixed pixel heights, so rows sized as a fraction of
   ...
   Only from bslib's sm breakpoint up: under it the cards are stacked in one
   column and Quarto sizes every row at minmax(1em, max-content), which this rule
   — an id, so it outweighs Quarto's own !important — used to override. The two
   fractions were then handed to a column of cards that no longer fill them, and
   each card was left with a band of empty space under its chart. */
...
/* Header and title type steps down as the viewport narrows. Browser zoom shrinks
   the viewport measured in CSS pixels, so zooming in walks down these steps just
   as narrowing the window does — which is the point: at the default sizes the
   subtitle wraps onto a second line and the logo crowds the nav links long
   before Bootstrap's 768px breakpoint folds them into the hamburger. */
@media (max-width: 1200px) {
  #quarto-dashboard-header .navbar-subtitle {
    font-size: 0.9rem;
  }

  #quarto-dashboard-header .navbar-logo {
    max-height: 34px;
  }
...
@media (max-width: 900px) {
  ...
  /* the axis ticks come down with the titles, or they start to dominate a chart
     that has itself shrunk with the viewport */
  .card-body svg [aria-label="tick"] text {
    font-size: 11px;
  }
...
# parse it and serialise it back out — across all elections that round trip
...
ojs_define(
  ...
  # next_election_date is the ISO form of next_election, and only the header
  # countdown reads it; an election without a fixed date leaves it out.
  election_meta = list(
    ST = list(
    ...
    SL = list(
      title = "Landtagswahl im Saarland 2027",
      seats = 51L,
      majority = 26L,
      last_election = "27. März 2022",
      next_election = "18. April 2027",
    ...
    SH = list(
      title = "Landtagswahl in Schleswig-Holstein 2027",
      seats = 69L,
      majority = 35L,
      last_election = "8. Mai 2022",
      next_election = "18. April 2027",
      next_election_date = "2027-04-18",
    ...
    NW = list(
      title = "Landtagswahl in Nordrhein-Westfalen 2027",
      seats = 181L,
      majority = 91L,
      last_election = "15. Mai 2022",
      next_election = "25. April 2027",
    ...
    HB = list(
      title = "Bürgerschaftswahl in Bremen 2027",
      seats = 87L,
      majority = 44L,
      last_election = "14. Mai 2023",
      next_election = "30. Mai 2027",
    ...
    NI = list(
      title = "Landtagswahl in Niedersachsen 2027",
      seats = 135L,
      majority = 68L,
      last_election = "9. Oktober 2022",
      next_election = "26. September 2027",
    ...
    BTW = list(
      ...
      # deliberately no next_election_date: the Bundestag term only fixes a
      # window, so the header shows that window instead of a countdown
      seat_method = "Sainte-Laguë/Schepers",
      hurdle = "5 %",
      pooling_days = 14L
    )
  ...
  party_colors = list(
    ...
    greens = "#46962b", fdp = "#ffed00", fw = "#005fa3", ssw = "#005ca9",
    left = "#be3075", bsw = "#6d1f99", others = "#aaaaaa"
  ),
  party_names = list(
    cdu = "Union", spd = "SPD", afd = "AfD",
    greens = "Grüne", fdp = "FDP", fw = "Freie Wähler", ssw = "SSW",
...
//| output: false
d3 = require("d3@7")
// Left to right by seating order in the Bundestag; the regional parties SSW
// and Freie Wähler keep their approximate place in that spectrum. "others"
// is not listed and sorts last wherever this order is applied.
party_order = ["bsw", "left", "spd", "greens", "ssw", "fdp", "fw", "cdu", "afd"]
...
// viableOnly=false keeps the politically excluded coalitions in: the Zeitverlauf
// lets a reader assemble any combination of parties, including the ones the rest
// of the site never reports.
cleanCoalitionHistory = (rows, viableOnly = true) => {
  const filtered = viableOnly
    ? rows.filter(d => isViableCoalition(coalitionParties(d.label)))
    : rows
  ...
  return html`
    <div>
      ...
      ${hasSeatInterval ? html`
        <p style="margin:10px 34px 0 34px;color:#333;font-size:1.08rem;line-height:1.35">
          ${intervalSentence}
        </p>
      ` : ""}
...
">
  ...
  <div style="
    ...
    background:#f7f9fc;
  ">
    <div><b>Überblick:</b> aktuelle Stimmenanteile, Mehrheitschancen und Hürdenwahrscheinlichkeiten.</div>
    <div style="margin-top:5px"><b>Sitzverteilung:</b> Verteilung möglicher Sitzanteile ausgewählter Parteikombinationen.</div>
    <div style="margin-top:5px"><b>Zeitverlauf:</b> Entwicklung der Mehrheitswahrscheinlichkeiten über die Zeit.</div>
    <div style="margin-top:5px"><b>Umfragedaten:</b> Umfragen, gepoolte Schätzung und Parteienwerte im Verlauf.</div>
    <div style="margin-top:5px"><b>Blog:</b> Analysen zu abgeschlossenen Wahlen.</div>
...
election_files = {
  return {
    ...
    SL: {
      coalitions        : FileAttachment("data/sl/coalition_probabilities.json"),
      party_data        : FileAttachment("data/sl/party_shares.json"),
      last_result_data  : FileAttachment("data/sl/last_results.json"),
      hurdle_data       : FileAttachment("data/sl/hurdle_probabilities.json"),
      pollster_data     : FileAttachment("data/sl/per_pollster.json"),
      history_data      : FileAttachment("data/sl/poll_history.json"),
      coal_history_data : FileAttachment("data/sl/coalition_history.json"),
      density_data      : FileAttachment("data/sl/coalition_densities.json")
    },
    SH: {
      coalitions        : FileAttachment("data/sh/coalition_probabilities.json"),
      party_data        : FileAttachment("data/sh/party_shares.json"),
      last_result_data  : FileAttachment("data/sh/last_results.json"),
      hurdle_data       : FileAttachment("data/sh/hurdle_probabilities.json"),
      pollster_data     : FileAttachment("data/sh/per_pollster.json"),
      history_data      : FileAttachment("data/sh/poll_history.json"),
      coal_history_data : FileAttachment("data/sh/coalition_history.json"),
      density_data      : FileAttachment("data/sh/coalition_densities.json")
    },
    NW: {
      coalitions        : FileAttachment("data/nw/coalition_probabilities.json"),
      party_data        : FileAttachment("data/nw/party_shares.json"),
      last_result_data  : FileAttachment("data/nw/last_results.json"),
      hurdle_data       : FileAttachment("data/nw/hurdle_probabilities.json"),
      pollster_data     : FileAttachment("data/nw/per_pollster.json"),
      history_data      : FileAttachment("data/nw/poll_history.json"),
      coal_history_data : FileAttachment("data/nw/coalition_history.json"),
      density_data      : FileAttachment("data/nw/coalition_densities.json")
    },
    HB: {
      coalitions        : FileAttachment("data/hb/coalition_probabilities.json"),
      party_data        : FileAttachment("data/hb/party_shares.json"),
      last_result_data  : FileAttachment("data/hb/last_results.json"),
      hurdle_data       : FileAttachment("data/hb/hurdle_probabilities.json"),
      pollster_data     : FileAttachment("data/hb/per_pollster.json"),
      history_data      : FileAttachment("data/hb/poll_history.json"),
      coal_history_data : FileAttachment("data/hb/coalition_history.json"),
      density_data      : FileAttachment("data/hb/coalition_densities.json")
    },
    NI: {
      coalitions        : FileAttachment("data/ni/coalition_probabilities.json"),
      party_data        : FileAttachment("data/ni/party_shares.json"),
      last_result_data  : FileAttachment("data/ni/last_results.json"),
      hurdle_data       : FileAttachment("data/ni/hurdle_probabilities.json"),
      pollster_data     : FileAttachment("data/ni/per_pollster.json"),
      history_data      : FileAttachment("data/ni/poll_history.json"),
      coal_history_data : FileAttachment("data/ni/coalition_history.json"),
      density_data      : FileAttachment("data/ni/coalition_densities.json")
    },
    BTW: {
...
// Headline line under the Stimmenanteile title. Whole calendar days from today,
// floored at zero so an election that has passed reads "Noch 0 Tage" instead of
// counting into the negative — local today against the date parsed as UTC
// midnight, as for `updated`. An election without a fixed date (the Bundestag
// term only fixes a window) states the window instead of a countdown.
election_date_label = {
  if (!cur_meta.next_election_date) return `Wahltermin: ${cur_meta.next_election}`

  const now = new Date()
  const days = Math.max(0, Math.round(
    (Date.parse(`${cur_meta.next_election_date}T00:00:00Z`)
      - Date.UTC(now.getFullYear(), now.getMonth(), now.getDate())) / 86400000
  ))
  return `Wahltermin am ${cur_meta.next_election} · Noch `
    + `${days.toLocaleString("de-DE")} ${days === 1 ? "Tag" : "Tage"} zur Wahl`
}
...
current_party_votes = Object.fromEntries(cleanShares(shares_source).map(d => [d.party, d.percent]))

// Label for a hand-picked set of parties, ordered like the coalition labels in
// the data are: the leading party first — the strongest current polling one
// unless a leader was picked — ties broken by seating order.
coalitionLabelFor = (parties, leader = null) =>
  labelFromParties(
    parties.slice().sort((a, b) =>
      d3.descending(a === leader, b === leader) ||
      d3.descending(current_party_votes[a] ?? 0, current_party_votes[b] ?? 0) ||
      d3.ascending(party_order.indexOf(a), party_order.indexOf(b))
    )
  )

// Holds the Zeitverlauf selector's combinations across its own re-runs. A cell
// without dependencies is evaluated once per page load, while the selector
// itself is rebuilt whenever the election or the institute changes. Written as a
// block: quarto's ojs parser leaves a bare object-literal cell undefined.
coalition_selection_store = {
  return { selections: null }
}

// Dash patterns for the second and third line sharing a colour with an earlier
// one, used by both the Zeitverlauf chart and the selector's swatches.
dash_patterns = ["6,4", "2,3"]

// The parties offered in the Zeitverlauf selector: everyone this election's
// pooled shares carry, Sonstige aside, in seating order.
selectable_parties = {
  const present = new Set(cleanShares(cur.party_data.party_shares).map(d => d.party))
  return party_order.filter(party => present.has(party))
}
...
methodik_steps = [
  ...
  },
  {
    ...
    lead: `Ausgezählt wird, in wie vielen Simulationen ein Bündnis mindestens
           ...
           Teilkombination daraus schon eine Mehrheit hätte.`
  }
]
```
...
<!-- tall enough for the selector with all three party combinations opened -->
## Row {height=1120px}
...
viewof hoveredCoalitionRow = {
  const rows  = cleanCoalitionHistory(coal_history_source, false)
  const dates = Array.from(new Set(rows.map(d => +d.date))).sort((a, b) => a - b)

  // The data holds one series per leadership ordering of a coalition, and a
  // selection names the ordering it wants (its largest party, unless another one
  // was picked) — so the line is that ordering's own series, not the sum over
  // them. Selections the backend never simulated (off the table politically, or
  // nowhere near a majority) have no series at all and are drawn at 0% rather
  // than left out.
  const probByKeyDate = d3.rollup(
    rows,
    v => d3.sum(v, d => d.probability),
    d => normCoalition(d.label),
    d => +d.date
  )
  const probByVariantDate = d3.rollup(
    rows,
    v => d3.sum(v, d => d.probability),
    d => `${normCoalition(d.label)}/${coalitionParties(d.label)[0]}`,
    d => +d.date
  )
  const probabilityAt = (key, t) =>
    (key.includes("/") ? probByVariantDate : probByKeyDate).get(key)?.get(t) ?? 0

  const series = selectedCoalitions
    .filter(d => d.parties.length)
    .map(({ parties, leader }) => {
      const lead  = parties.includes(leader) ? leader : null
      const key   = lead ? `${partyKey(parties)}/${lead}` : partyKey(parties)
      const label = coalitionLabelFor(parties, lead)
      return {
        key,
        label,
        color: party_colors[coalitionParties(label)[0]] || "#aaa",
        // sorted by date so Plot.line connects the points in chronological order
        rows: dates.map(t => ({ key, label, date: new Date(t), probability: probabilityAt(key, t) }))
      }
    })

  // several selections can be led by the same party, and would then be lines in
  // the same colour; every one after the first gets its own dash so they stay
  // tellable apart
  series.forEach((d, i) => {
    const sameColor = series.slice(0, i).filter(other => other.color === d.color).length
    d.dash = sameColor ? dash_patterns[sameColor - 1] : undefined
  })

  // one row per date, one column per selection — used for the crosshair, which
  // tracks the nearest date across all selected lines at once
  const wide = dates.map(t => Object.assign(
    { date: new Date(t) },
    ...series.map(d => ({ [d.key]: probabilityAt(d.key, t) }))
  ))
  ...
  const plot = Plot.plot({
    ...
    x: {
      ...
      interval: "day",
      tickPadding: 12,
      // fixed to the full available history, so the axis does not jump around
      // while combinations are put together button by button
      domain: dates.length ? [new Date(dates[0]), new Date(dates.at(-1))] : undefined,
    ...
    marks: [
      Plot.ruleY([0]),
      // one mark per selection rather than a colour scale over all of them, so
      // the dashed second line stays a per-series setting
      ...series.flatMap(d => [
        Plot.line(d.rows, {
          x: "date",
          y: "probability",
        ...
        // A line needs two points to render anything, so a history of a single
        // date (some institutes have polled a state exactly once) gets a dot, or
        // the selection would vanish.
        ...(dates.length === 1
          ? [Plot.dot(d.rows, { x: "date", y: "probability", fill: d.color, stroke: "white", strokeWidth: 1, r: 4 })]
          : [])
      ])
    ]
  })

  if (!lastRow || !series.length) return plot
  ...
  const items = series.map(d => {
    ...
    return { key, label, link, dot, text }
  })
...
viewof selectedCoalitions = {
  // Up to three hand-built party combinations, each one a set of parties plus the
  // party leading it. The first starts on the coalition currently most likely to
  // hold a majority, so the chart is never empty on load.
  const ranked = leading_variants.slice().sort((a, b) =>
    d3.descending(a.probability, b.probability) || d3.descending(a.vote_share, b.vote_share)
  )
  const maxSelections = 3

  // This cell reads the election's and the institute's data, so switching either
  // one rebuilds it from scratch. Carry the combinations over through the store,
  // dropping the parties that do not run in the election switched to.
  const stored = (coalition_selection_store.selections ?? [])
    .map(({ parties, leader }) => ({
      parties: new Set(parties.filter(party => selectable_parties.includes(party))),
      leader
    }))

  const selections = stored.some(selection => selection.parties.size) ? stored : [{
    parties: new Set(ranked.length ? coalitionParties(ranked[0].label) : []),
    leader: null
  }]

  // FDP yellow and AfD light blue are illegible as text on white and as white
  // text on their own fill, so light party colours get a darkened outline and
  // dark lettering while keeping their true colour as the active background.
  const isLight  = color => d3.hcl(color).l > 70
  const inkOf    = color => isLight(color) ? d3.hcl(color).darker(1.5).formatHex() : color
  const onFillOf = color => isLight(color) ? "#1c1c1b" : "white"

  // The combination is always shown as one leadership ordering; unless another
  // one is picked, that is the ordering led by its strongest polling party.
  const largestOf = parties => parties.slice().sort((a, b) =>
    d3.descending(current_party_votes[a] ?? 0, current_party_votes[b] ?? 0) ||
    d3.ascending(party_order.indexOf(a), party_order.indexOf(b))
  )[0] ?? null

  const leadOf = selection => {
    const parties = selectable_parties.filter(party => selection.parties.has(party))
    return parties.includes(selection.leader) ? selection.leader : largestOf(parties)
  }
  const partiesOf = ({ parties }) => selectable_parties.filter(party => parties.has(party))
  const colorOf  = selection => {
    const parties = partiesOf(selection)
    if (!parties.length) return "#aaa"
    return party_colors[coalitionParties(coalitionLabelFor(parties, leadOf(selection)))[0]] || "#aaa"
  }

  const valueOf = () => selections.map(selection => ({
    parties: partiesOf(selection),
    leader: leadOf(selection)
  }))

  const form = html`<div class="kombi-picker"></div>`

  // Only an explicitly picked leader is carried over; left at null, the largest
  // party of the combination leads it, recomputed for whatever is switched to.
  const persist = () => {
    coalition_selection_store.selections = selections.map(selection => ({
      parties: partiesOf(selection),
  ...
  const render = () => {
    form.textContent = ""
    form.append(html`<div class="kombi-picker__title">Parteikombination auswählen</div>`)

    // mirrors the chart: combinations in the same colour are told apart by dashing
    // every one after the first, and the swatch shows which line is which
    const drawn = selections.filter(selection => selection.parties.size)
    const dashOf = selection => {
      const i = drawn.indexOf(selection)
      if (i < 1) return ""
      const sameColor = drawn.slice(0, i).filter(other => colorOf(other) === colorOf(selection)).length
      return sameColor ? dash_patterns[sameColor - 1] : ""
    }

    selections.forEach((selection, i) => {
      const { parties } = selection
      const chosen = partiesOf(selection)
      const color  = colorOf(selection)
      const card   = html`<div class="kombi-card"></div>`
      const head   = html`<div class="kombi-card__head"></div>`

      head.append(html`<span class="kombi-card__index">Kombination ${i + 1}</span>`)
      head.append(chosen.length
        ? html`<span class="kombi-card__name">
            <svg width="20" height="8" aria-hidden="true"><line x1="1" y1="4" x2="19" y2="4"
              stroke="${color}" stroke-width="3" stroke-linecap="round"
              stroke-dasharray="${dashOf(selection)}"></line></svg>
            ${coalitionLabelFor(chosen, leadOf(selection))}
          </span>`
        : html`<span class="kombi-card__name kombi-card__name--empty">Parteien auswählen</span>`)
      head.append(html`<span class="kombi-card__spacer"></span>`)

      // Which ordering of the combination to show: the coalitions are simulated
      // per leading party, and summing those orderings up (the default) answers
      // a different question than picking one of them.
      if (chosen.length > 1) {
        const select = html`<select>
          ${chosen.map(party => html`<option value="${party}" style="color:${inkOf(party_colors[party] || "#aaa")}">${party_to_label[party] || party}</option>`)}
        </select>`
        select.value = leadOf(selection)
        // the picked party carries its own colour, which is also the colour of
        // the line it puts on the chart
        const leadColor = inkOf(party_colors[leadOf(selection)] || "#aaa")
        select.style.color       = leadColor
        select.style.borderColor = leadColor
        select.style.fontWeight  = 600

        select.onchange = () => {
          selection.leader = select.value
          render()
          emit()
        }
        head.append(html`<label class="kombi-leader">Größte Partei ${select}</label>`)
      }

      if (selections.length > 1) {
        const drop = html`<button type="button" class="kombi-drop" title="Kombination entfernen">✕</button>`
        drop.onclick = () => {
      ...
      const pills = html`<div class="kombi-parties"></div>`

      for (const party of selectable_parties) {
        const partyColor = party_colors[party] || "#aaa"
        const isOn = parties.has(party)
        const btn  = html`<button type="button" class="kombi-pill">${party_to_label[party] || party}</button>`

        btn.style.borderColor = inkOf(partyColor)
        btn.style.background  = isOn ? partyColor : "white"
        btn.style.color       = isOn ? onFillOf(partyColor) : inkOf(partyColor)

        btn.onclick = () => {
          if (isOn) parties.delete(party)
          else parties.add(party)
          // a leader that just left the combination cannot lead it any more
          if (!parties.has(selection.leader)) selection.leader = null
          render()
          emit()
        }
    ...
    const actions = html`<div class="kombi-actions"></div>`

    if (selections.length < maxSelections) {
      const addBtn = html`<button type="button" class="kombi-action">+ Weitere Parteikombination hinzufügen</button>`
      addBtn.onclick = () => {
        selections.push({ parties: new Set(), leader: null })
        render()
        emit()
      }
    ...
    const deselectBtn = html`<button type="button" class="kombi-action">Auswahl entfernen</button>`
    deselectBtn.onclick = () => {
      selections.splice(1)
      selections[0].parties.clear()
      selections[0].leader = null
      render()
      emit()
    }
    actions.append(deselectBtn)
...
<div class="blog-index">
  <header class="blog-index-header">
    <h2>Analysen zu den Wahlen</h2>
    <p>Einordnung von Umfragen, KOALA-Schätzungen und amtlichen Ergebnissen.</p>
  </header>

  <div class="blog-list" aria-label="Blogbeiträge">
    <a class="blog-post" href="blog/202609_ltwbe/index.html">
      <div class="blog-post-media">
        <img src="blog/202609_ltwbe/election_result.png"
             alt="Vorläufiges Zweitstimmenergebnis der Abgeordnetenhauswahl in Berlin 2026">
      </div>
      <div>
        <div class="blog-post-meta">
          <time datetime="2026-09-23">23. September 2026</time>
          <span>Abgeordnetenhauswahl</span>
        </div>
        <h3>Abgeordnetenhauswahl in Berlin</h3>
        <p class="blog-post-summary">
          Wie gut lagen die letzten Wahlumfragen und die KOALA-Schätzung im
          Vergleich zum vorläufigen Wahlergebnis?
        </p>
      </div>
    </a>

    <a class="blog-post" href="blog/202609_ltwmv/index.html">
      <div class="blog-post-media">
        <img src="blog/202609_ltwmv/election_result.png"
             alt="Vorläufiges Zweitstimmenergebnis der Landtagswahl in Mecklenburg-Vorpommern 2026">
      </div>
      <div>
        <div class="blog-post-meta">
          <time datetime="2026-09-23">23. September 2026</time>
        ...
        <h3>Landtagswahl in Mecklenburg-Vorpommern</h3>
        <p class="blog-post-summary">
          Wie gut lagen die letzten Wahlumfragen und die KOALA-Schätzung im
          Vergleich zum vorläufigen Wahlergebnis?
        </p>
    ...
    <a class="blog-post" href="blog/202609_ltwsa/index.html">
      <div class="blog-post-media">
        <img src="blog/202609_ltwsa/election_result.png"
             alt="Vorläufiges Zweitstimmenergebnis der Landtagswahl in Sachsen-Anhalt 2026">
      </div>
      <div>
        <div class="blog-post-meta">
        ...
        <h3>Landtagswahl in Sachsen-Anhalt</h3>
        <p class="blog-post-summary">
          Wie gut lagen die letzten Wahlumfragen und die KOALA-Schätzung im
          Vergleich zum vorläufigen Wahlergebnis?
        </p>
