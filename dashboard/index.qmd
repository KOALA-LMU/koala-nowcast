...
methodik_steps = [
  ...
  },
  {
    ...
    lead: `Ausgezählt wird, in wie vielen Simulationen ein Bündnis mindestens
           ${cur_meta.majority} Sitze erreicht.`
  }
]
```
...
```{ojs}
//| output: false
// Institutes whose polls actually feed the currently selected election, newest
// poll first — the Datenbasis section names them instead of listing the static
// config roster, which also contains institutes that never polled this race.
methodik_institutes = {
  const latest = new Map()
  for (const d of cur.pollster_data.per_pollster) {
    const prev = latest.get(d.pollster)
...
```{ojs}
//| output: false
// One card per pipeline step. Kept as data so the markup below stays a single
