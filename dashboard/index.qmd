...
//| label: chart-vote-shares
{
  ...
  // The eight party names sit side by side under the bars, so they only fit
  // while the chart is wide. Measured on the tightest neighbours (AfD and
  // Sonstige): the gap between them is 94px at a 1130px-wide chart, 34px at
  // 650px, 4px at 410px, and it closes entirely just below 380px. `width` is
  // Quarto's reactive viewport width, and this chart tracks it at a constant
  // offset of 310px — so 830px of viewport is the ~520px-wide chart where the
  // gap is down to about 18px, and the names go on the diagonal from there.
  //
  // Reading `width` also makes this cell re-run whenever the viewport changes,
  // which covers zooming as well as resizing: browser zoom shrinks the viewport
  // measured in CSS pixels, so zooming in rotates the labels exactly as making
  // the window narrower would.
  const rotateTicks = width < 830
  ...
  return Plot.plot({
    ...
    marginTop: 40,
    // A rotated name reaches about 40px below the axis, so the value and change
    // rows underneath need pushing down and the margin needs to grow with them.
    marginBottom: rotateTicks
      ? (hasLastResult ? 104 : 74)
      : (hasLastResult ? 86 : 55),
    marginLeft: 55,
    marginRight: 25,
    height: 360,
