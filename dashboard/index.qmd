...
# The one date the countdown and the archiving share: election_date in
# config/elections/*.yml is also what stops scraping and pooling once it has
# passed (election_is_over() in scripts/pending_configs.R), so the site cannot
# count down to a day other than the one it archives on. A config without the
# key — the Bundestagswahl, whose term fixes a window rather than a day —
# yields NULL, which the Filter() below drops, and the header then shows
# next_election as text instead of a countdown.
# Rendered from dashboard/, hence the ".." — nothing else here reads the repo.
election_date <- function(id)
  yaml::yaml.load_file(file.path("..", "config", "elections", paste0(id, ".yml")))$election_date
...
election_meta <- lapply(
  list(
    ST = list(
      ...
      next_election_date = election_date("ltw_st"),
    ...
    MV = list(
      ...
      next_election_date = election_date("ltw_mv"),
      seat_method = "Hare-Niemeyer",
      hurdle = "5 %",
      pooling_days = 14L
    ),
    BE = list(
      ...
      next_election = "20. September 2026",
      next_election_date = election_date("ltw_be"),
      seat_method = "Hare-Niemeyer",
      hurdle = "5 %",
      pooling_days = 14L
    ),
    BTW = list(
      ...
      next_election_date = election_date("btw"),
      seat_method = "Sainte-Laguë/Schepers",
      hurdle = "5 %",
      pooling_days = 14L
    )
  ),
  function(m) Filter(Negate(is.null), m)
)
