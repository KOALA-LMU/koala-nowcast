...

{
  ...
  const coalitionPlot = (rows, title = null) => {
    ...
    // Plot defaults to 640px, which left the right third of the card empty. The
    // card is one of two columns beside the sidebar, so mirror the density
    // chart's viewport arithmetic and halve it; never below the old default.
    width: Math.max(640, (window.innerWidth - 430) / 2 - 40),
    x: {
  ...
  const moreToggle = remaining.length
    ? html`<details class="more-coalitions" style="margin:8px 0 0 ${axisMargin}px;font-size:0.9em;color:#555">
        <summary style="cursor:pointer;font-weight:700">
          Weitere ${remaining.length} Parteikombinationen anzeigen
        </summary>
        <div style="margin-top:8px;margin-left:${-axisMargin}px">
